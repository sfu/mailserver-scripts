...
sub new {
	...
	$self->{isProd} = !$isTest;
	$self->{baseUrl} = "http://icat-rob-macpro.its.sfu.ca/cgi-bin/WebObjects/Maillist.woa/-60666/ra/";
	$self->{baseUrl} = "https://rest.maillist.sfu.ca/" if $self->{isProd};
#	$self->{baseUrl} = "https://amaint.sfu.ca/cgi-bin/WebObjects/Maillist.woa/ra/" if $self->{isProd};
	_stdout("baseUrl is ".$self->{baseUrl});
	$self->login();
	unless ($self->{token}) {
...
sub getMaillistById {
	...
	_stderr("getMaillist: Getting $url");
	my $mldata = _httpGet($url);
...
sub getMaillistByName {
    ...
    my $url = $self->{baseUrl} . "maillists.json?name=$name&sfu_token=" . $self->{token};
	_stderr("getMaillist: Getting $url");
	my $mldata = _httpGet($url);
    return new MLRestMaillist($self, $mldata);
}
...
sub getMaillistsByNameWildcard {
    ...
    return $self->getMaillistsWithFilter("name=$name");
}
...
sub getMembersForMaillist {
    ...
    my $memdata = _httpGet($membersUri);
...
sub createCourselist {
    ...
    push @params, @optional if defined @optional;
