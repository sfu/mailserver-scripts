...
sub canonicalAllowedToSend {
	...
	if (MLMail::hasNoDomain($sender)) {
		# local
		_stdout( "canonicalAllowedToSend:name: ".$sender ) if $main::TEST;
		if (MLMail::isMaillist($sender)) {
			return $self->maillistIsInAllowList($sender);
		} 
...
sub expandedList {
	...
	foreach $member (keys %{$members}) {
		if ($members->{$member}{type} == MAILLIST) {
			_stdout( "expandedList: $member is a maillist" ) if $main::TEST;
			next if $seen->{$member};
			my $mlc = new MLCache($member);
			next unless $mlc;
			push @addrs, $mlc->expandedList($seen);
		} else {
			push @addrs, $member;
		}
	}
